...
class NumpyFileCheckpointer(utils.Checkpointer):
    ...
    def __init__(self, config: config_dict.ConfigDict, mode: str):
        self._checkpoint_file = os.path.join(config.checkpoint_dir, "checkpoint.npy")
...
def default_color_augmentation_fn(
    ...
    frames = tf.cond(
        pred=tf.less(coin_toss_color_augment, tf.cast(prob_color_augment, tf.float32)),
        true_fn=lambda: color_augment(frames),
        false_fn=lambda: frames,
    )

    # Eventually applies color drop.
    coin_toss_color_drop = tf.random.uniform([], minval=0, maxval=1, dtype=tf.float32)
    frames = tf.cond(
...
def add_default_data_augmentation(ds: tf.data.Dataset) -> tf.data.Dataset:
    return ds.map(default_color_augmentation_fn, num_parallel_calls=tf.data.AUTOTUNE)
